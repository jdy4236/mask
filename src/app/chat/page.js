...
export default function ChatPage() {
    ...
    return (
        <div className="flex flex-col min-h-screen bg-background-color text-text-color">
            ...
            <div className={`flex-grow p-6 bg-background-color ${styles.container}`}>
                {error && <p className="text-error-color mb-4 text-center">{error}</p>}

                <div className={`flex flex-col bg-secondary-background-color rounded shadow-lg w-full max-w-md ${styles.chatContainer}`}>
                    <header className="bg-background-color text-text-color py-4 px-6 flex justify-between rounded-t">
                        <div>
                            <h1 className="text-lg font-bold">채팅 방: {roomId}</h1>
                        </div>
                        <button
                            onClick={generateInvite}
                            className="bg-input-background-color text-text-color px-4 py-2 rounded shadow hover:bg-accent-color"
                        >
                            초대 링크 생성
                        </button>
                    </header>
                    ...
                    <footer className="bg-background-color p-4 flex">
                    ...
                    {showPasswordPrompt && (
                        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
                            <div className="bg-secondary-background-color p-6 rounded shadow-md w-80">
                                <h2 className="text-xl font-bold mb-4">비밀방 비밀번호 입력</h2>
                                <input
                                    type="password"
                                    placeholder="비밀번호"
                                    className="border p-2 rounded mb-4 w-full bg-input-background-color text-text-color"
                                    value={password}
                                    onChange={(e) => setPassword(e.target.value)}
                                />
                                <button
                                    className="px-4 py-2 bg-input-background-color text-text-color rounded w-full hover:bg-accent-color"
